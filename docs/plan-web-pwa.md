...
- **Потягивание вниз для обновления данных не работает на вебе, известный gap
  (найдено 2026-09-20).** `RefreshControl` (`dashboard.tsx`, `CoachHome.tsx`) на
  `react-native-web` — пустышка, рендерит просто `<View>` без обработки жеста
  (`node_modules/react-native-web/dist/exports/RefreshControl/index.js`), поэтому
  свайп вниз никогда не обновлял данные ни в обычной вкладке браузера, ни тем более
  в установленном standalone-PWA (там ещё и браузерного «потяни — перезагрузится»
  нет, весь UI хрома убран `"display": "standalone"`). Сейчас на вебе обновить
  данные можно только полной перезагрузкой приложения. Осознанно не чиним сейчас —
  решение отложено. Кандидат фикса на будущее: кнопка «Обновить» в шапке под
  `Platform.OS === 'web'`, вызывающая тот же `qc.invalidateQueries()`, что и
  текущий `onRefresh`.
