...
// На вебе шрифты Google подгружаются асинхронно (FontFace). Пока они не загрузились
// — и если сеть медленная — нужен системный фолбэк, иначе текст рисуется дефолтной
// засечкой. На нативе fontFamily — одно имя шрифта; список через запятую там ломает
// рендер, поэтому фолбэк добавляем только на web.
//
// process.env.EXPO_OS подставляет babel-preset-expo на этапе сборки — платформенная
// ветка без импорта Platform: этот модуль грузится очень рано, и лишний импорт
// react-native здесь ловит circular-import TDZ.
const WEB_FONT_FALLBACK =
  ', system-ui, -apple-system, "Segoe UI", Roboto, Helvetica, Arial, sans-serif';
const withFallback = (name: string) =>
  process.env.EXPO_OS === 'web' ? name + WEB_FONT_FALLBACK : name;
...
export const defaultTheme: Theme = {
  ...
  font: {
    display: withFallback('Unbounded_600SemiBold'),
    regular: withFallback('Inter_400Regular'),
    medium: withFallback('Inter_500Medium'),
    semibold: withFallback('Inter_600SemiBold'),
    bold: withFallback('Inter_700Bold'),
  },
};
