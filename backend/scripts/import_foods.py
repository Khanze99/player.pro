...
Два режима:
  ...
  make foods            — базовый выверенный набор (~50 продуктов), работает сразу
...
from app.models.enums import FoodSource  # noqa: E402
...
async def load_curated() -> int:
    async with AsyncSessionLocal() as db:
        added = 0
        for name, brand, kcal, protein, fat, carbs, fiber, serving, serving_name in CURATED:
            exists = await db.execute(
                select(FoodItem.id).where(FoodItem.name == name, FoodItem.source == FoodSource.curated)
            )
...
async def import_off(path: Path, limit: int, country: str) -> int:
    """Потоковый импорт дампа: файл на гигабайты, в память он не влезет.

    Берём только строки с осмысленной энергетикой — записи без нутриентов
    в дневнике бесполезны и только засоряют поиск.
    """
    ...
    async with AsyncSessionLocal() as db:
        ...
        with opener(path, "rt", encoding="utf-8", errors="replace", newline="") as handle:
            ...
            for row in reader:
                ...
                if not name or len(name) > 255 or kcal is None or kcal == 0 or kcal > 900:
                    continue

                db.add(
                    FoodItem(
                        source=FoodSource.open_food_facts,
                        barcode=(row.get("code") or "").strip()[:32] or None,
                        name=name,
                        brand=(row.get("brands") or "").strip()[:128] or None,
                        kcal_100g=kcal,
...
def main() -> None:
    ...
    parser.add_argument("--limit", type=int, default=50000, help="максимум записей из дампа")
    parser.add_argument("--country", default="russia", help="фильтр по стране; пусто — без фильтра")
    args = parser.parse_args()

    if args.off:
        print(f"Импорт Open Food Facts из {args.off} (лимит {args.limit}, страна: {args.country})")
        added = asyncio.run(import_off(args.off, args.limit, args.country))
        print(f"Готово. Добавлено записей: {added}")
    else:
        print("Загрузка базового выверенного справочника")
        added = asyncio.run(load_curated())
        print(f"Готово. Добавлено продуктов: {added} (всего в наборе {len(CURATED)})")
