...
Коды входа уходят почтой: канал задаётся `OTP_EMAIL_CHANNEL=email` и SMTP-настройками
в `infra/.env` (Yandex Cloud Postbox). Вход по телефону пока не работает — SMS-провайдера
нет, канал `OTP_PHONE_CHANNEL` стоит в `log`.

Без настроенного SMTP войти на стенде можно только при `DEBUG=true`: тогда
`POST /api/v1/auth/otp/request` возвращает OTP-код прямо в ответе. Это делает стенд
открытым для всех, кто до него дотянется, поэтому по умолчанию `DEBUG=false` —
включать лишь на закрытом стенде. Чек-лист прода — в `docs/deploy.md`.
...
Версия приложения — две цифры: **первая меняется при новой функциональности, вторая при
исправлениях** (`1.0` → `1.1` → `2.0`). Живёт в `mobile/app.json`, поле `version`.

Третья цифра не нужна: patch-версия в semver существует ради потребителей кода, а
совместимость API у нас выражена префиксом `/api/v1` — ломающие изменения контракта
уедут в `/api/v2`, а не в номер релиза.

Номер сборки (`versionCode` у Android, `buildNumber` у iOS) — отдельный счётчик, магазины
требуют, чтобы он строго рос при каждой загрузке. Вручную его не трогаем: в `eas.json`
включены `cli.appVersionSource: "remote"` и `autoIncrement` на профилях сборки, счётчик
ведёт EAS.

`mobile/package.json` остаётся в формате semver (`1.0.0`) — npm требует три компонента,
но пакет приватный и наружу не публикуется, пользователь этот номер не видит.
